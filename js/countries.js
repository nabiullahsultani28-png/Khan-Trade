(function () {
  ...
  const countriesData = [
    ...
    { name: "Congo (Congo-Brazzaville)", code: "CG", dial: "+242" },
    { name: "Congo (Democratic Republic)", code: "CD", dial: "+243" },
    { name: "Costa Rica", code: "CR", dial: "+506" },
    { name: "Croatia", code: "HR", dial: "+385" },
    { name: "Cuba", code: "CU", dial: "+53" },
    { name: "Cyprus", code: "CY", dial: "+357" },
    { name: "Czechia (Czech Republic)", code: "CZ", dial: "+420" },
    ...
    { name: "Myanmar (Burma)", code: "MM", dial: "+95" },
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    const dialCodeInput = document.getElementById('phoneCode') || document.querySelector('input[aria-label="Country Code"]');

    if (!container || !searchInput || !hiddenInput || !listEl) return;
    ...
    const renderList = (filter = '') => {
      ...
      const filtered = countriesData.filter(c => c.name.toLowerCase().includes(term));
      ...
      filtered.forEach(c => {
        ...
        if (hiddenInput.value === c.code) {
          opt.classList.add('selected');
        }

        opt.addEventListener('mousedown', (e) => {
          // mousedown fires before blur, preventing the input blur from closing it prematurely
          e.preventDefault();
          selectCountry(c);
        });
    ...
    const selectCountry = (c) => {
      ...
      if (dialCodeInput) {
        dialCodeInput.value = c.dial;
      }
      closeDropdown();
      
      // Clear validation state since we now have a value
    ...
    const closeDropdown = () => {
      ...
      // If closed and the text doesn't match the selected hidden value, revert or clear
      const selectedObj = countriesData.find(c => c.code === hiddenInput.value);
      if (selectedObj) {
        searchInput.value = selectedObj.name;
      } else {
        searchInput.value = '';
        hiddenInput.value = '';
    ...
    searchInput.addEventListener('input', (e) => {
      ...
      hiddenInput.value = ''; // clear hidden value if typing
    });
